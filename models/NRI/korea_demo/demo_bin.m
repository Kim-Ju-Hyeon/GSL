...
act_tmp=nan(nsteps/100,N);
lam_tmp=nan(nsteps/100,N);
spk_tmp=nan(nsteps/100,N);

act_bin=nan(nsteps/10,N);
lam_bin=nan(nsteps/10,N);
spk_bin=nan(nsteps/10,N);
...
for j=1:niter
    ...
    %     drawnow
        ...
        for k=1:N
            if i == 1
                %act_tmp=activation(k:N:end);
                lam_tmp=lambda(k:N:end);
                spk_tmp=spikes(k:N:end);
            else
                %act_tmp=act_tmp+activation(i:N:end);
...
for i=1:N
    if i == 1
        %act_bin=activation(i:N:end);
        lam_bin=lambda(i:N:end);
        spk_bin=spikes(i:N:end);
    else
        %act_bin=act_bin+activation(i:N:end);
        lam_bin=lam_bin+lambda(i:N:end);
        spk_bin=spk_bin+spikes(i:N:end);
    end
end
...
%save('act_all.mat', 'activations', '-v7.3'); % If ring
save('spk_bin.mat', 'spikes', '-v7.3');
save('lam_bin.mat', 'lambda', '-v7.3'); % If LNP
